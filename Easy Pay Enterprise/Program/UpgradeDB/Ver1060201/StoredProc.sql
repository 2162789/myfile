if exists(select 1 from sys.sysprocedure where proc_name = 'FGetLveTotalEntitlement' and user_name(creator) = 'DBA') then
   drop function DBA.FGetLveTotalEntitlement
end if
;
...
begin
  ...
  if Out_RoundingMethod = 'Fixed' then
    if(Out_TotalEntRoundMethod = 0) then return "TRUNCATE"(Cast((In_EntEarned+In_EntAdjEarned+In_BFEarned-In_BFForfeit) as money),2)
    end if;
    if(Out_TotalEntRoundMethod = 1) then return "TRUNCATE"(Cast((In_EntEarned+In_EntAdjEarned) as money),2)+In_BFEarned-In_BFForfeit
    end if;
    if(Out_TotalEntRoundMethod = 2) then return "TRUNCATE"(Cast((In_EntEarned) as money),2)+In_EntAdjEarned+In_BFEarned-In_BFForfeit
    end if
  end if;
