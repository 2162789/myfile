...
BEGIN
    ...
    */
    ...
    (
		...
		Union  
		Select Distinct 'Location' As [Type], CountryCode As [Code], CountryCode As [Description], Cast(1 As bit) As [Active] From Calendar
		Where (In_Type = '') Or (In_Type ='Location' And (In_Code = CountryCode Or In_Code = ''))
		Union  
...
if exists(select 1 from sys.systrigger where trigger_name = 'ESSUpdateEmpeeWkCalen') then
   drop trigger ESSUpdateEmpeeWkCalen
end if;
CREATE TRIGGER DBA.ESSUpdateEmpeeWkCalen after update of CalendarId
order 36 ON DBA.EmpeeWkCalen
