/* Add the security setup for Loan Report */
if (not exists (select * from ModuleScreenGroup where ModuleScreenId = 'LoanPhilipGeneric') and FGetDBCountry() not in ('Philippines','Vietnam')) then
...
if exists (select * from sys.sysviews where viewname = 'View_TMS_AllowanceID') then
  alter view View_TMS_AllowanceID as
  ...
  , FormulaSubCategory FROM Formula WHERE FormulaSubCategory IN ('Allowance','Deduction', 'Reimbursement') AND FormulaActive=1 AND FormulaType IN ('Free', 'Formula');
else
  create view View_TMS_AllowanceID as
  SELECT FormulaId as PayElementId, FormulaDesc as PayElementDesc, FormulaType, (IF (SELECT 1 FROM FormulaProperty WHERE FormulaId=Formula.FormulaId and KeywordId = 'GRPCode') IS NULL THEN 0 ELSE 1 ENDIF) AS IsGRPCode 
  , FormulaSubCategory FROM Formula WHERE FormulaSubCategory IN ('Allowance','Deduction', 'Reimbursement') AND FormulaActive=1 AND FormulaType IN ('Free', 'Formula');
end if;

if exists(select * from sys.sysprocedure where proc_name = 'FGetLveCreditExpired') then
   drop function FGetLveCreditExpired;
end if;
