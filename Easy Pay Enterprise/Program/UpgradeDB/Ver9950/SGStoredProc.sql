...
create procedure
DBA.ASQLYEProcessA8B(in In_PersonalSysID integer,in In_YEYear integer,in In_Operation char(20))
begin
  declare Out_TotalEESOPTaxExempt double;
  declare Out_TotalCSOPTaxExempt double;
  declare Out_TotalNSOPTaxExempt double;
  ...
  */
  update A8BRecord set
    ...
    CSOPTaxExempt = 0,
    NSOPTaxExempt = 0,
    NoTaxExempt = Round("Truncate"((MktValueExerciseStock-ExercisePriceStock)*SharesAcquired,3),2) where
    PersonalSysId = In_PersonalSysId and
    YEYear = In_YEYear and StockOptionType = 'ESOP';
  update A8BRecord set
    StockGains = Round(NoTaxExempt,2) where
  ...
  */
  update A8BRecord set
    EESOPTaxExempt = Round("Truncate"((MktValueExerciseStock-MktValueStockGrant)*SharesAcquired,3),2),
    CSOPTaxExempt = 0,
    NSOPTaxExempt = 0,
    NoTaxExempt = Round("Truncate"((MktValueStockGrant-ExercisePriceStock)*SharesAcquired,3),2) where
    PersonalSysId = In_PersonalSysId and
    YEYear = In_YEYear and StockOptionType = 'EESOP';
  update A8BRecord set
    StockGains = Round(EESOPTaxExempt+NoTaxExempt,2) where
  ...
  */
  update A8BRecord set
    EESOPTaxExempt = 0,
    CSOPTaxExempt = Round("Truncate"((MktValueExerciseStock-MktValueStockGrant)*SharesAcquired,3),2),
    NSOPTaxExempt = 0,
    NoTaxExempt = Round("Truncate"((MktValueStockGrant-ExercisePriceStock)*SharesAcquired,3),2) where
    PersonalSysId = In_PersonalSysId and
    YEYear = In_YEYear and StockOptionType = 'CSOP';
  ...
  */
  update A8BRecord set
    EESOPTaxExempt = 0,
    CSOPTaxExempt = 0,
    NSOPTaxExempt = Round("Truncate"((MktValueExerciseStock-MktValueStockGrant)*SharesAcquired,3),2),
    NoTaxExempt = Round("Truncate"((MktValueStockGrant-ExercisePriceStock)*SharesAcquired,3),2) where
    PersonalSysId = In_PersonalSysId and
    YEYear = In_YEYear and StockOptionType = 'NSOP';
  update A8BRecord set
...
if exists(select 1 from sys.sysprocedure where proc_name = 'FGetPayPeriodMedisaveAdditional') then
   drop procedure FGetPayPeriodMedisaveAdditional
end if;

create function
DBA.FGetPayPeriodMedisaveAdditional(in In_EmployeeSysId integer,in In_PayGroupId char(20),in In_Year integer,in In_Period integer)
returns double
begin
  declare Out_MedisaveAdditional double;
  if(In_PayGroupId = '') then
    select SUM(MedisaveAdditional) into Out_MedisaveAdditional
    ...
    select SUM(MedisaveAdditional) into Out_MedisaveAdditional
      from PayPeriodRecord join PeriodPolicySummary where
      PayPeriodRecord.EmployeeSysId = In_EmployeeSysId and
      PayPeriodRecord.PayPayGroupId = In_PayGroupId and
      PayPeriodRecord.PayRecYear = In_Year and
      PayPeriodRecord.PayRecPeriod = In_Period
      group by PayPeriodRecord.EmployeeSysId
  end if;
  if Out_MedisaveAdditional is null then
    return 0
  else
    return Out_MedisaveAdditional
  end if
end
;

if exists(select 1 from sys.sysprocedure where proc_name = 'FGetPayPeriodMedisaveOrdinary') then
   drop procedure FGetPayPeriodMedisaveOrdinary
end if;

create function
DBA.FGetPayPeriodMedisaveOrdinary(in In_EmployeeSysId integer,in In_PayGroupId char(20),in In_Year integer,in In_Period integer)
returns double
begin
  declare Out_MedisaveOrdinary double;
  if(In_PayGroupId = '') then
    select SUM(MedisaveOrdinary) into Out_MedisaveOrdinary
      from PayPeriodRecord join PeriodPolicySummary where
      PayPeriodRecord.EmployeeSysId = In_EmployeeSysId and
      PayPeriodRecord.PayRecYear = In_Year and
      PayPeriodRecord.PayRecPeriod = In_Period
  ...
  if Out_MedisaveOrdinary is null then
    return 0
  else
    return Out_MedisaveOrdinary
  end if
end
;
