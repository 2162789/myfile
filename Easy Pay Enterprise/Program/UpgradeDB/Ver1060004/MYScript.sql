READ UpgradeDB\Ver1060004\MY_Holidays.sql;
...
if exists(select 1 from sys.sysprocedure where proc_name = 'FGetMalTaxRecordEmployeeSysId') then 
  drop function FGetMalTaxRecordEmployeeSysId
end if
;
  ...
  if(TaxEmploymentCount = 1) then
    ...
    */
    select first MalTaxEESysId into Out_EmployeeSysId from MalTaxEmployee where
      PersonalSysId = In_PersonalSysId and
      MalTaxYear = In_MalTaxYear order by ToPayRecYear desc,ToPayRecPeriod desc, FromPayRecYear desc, FromPayRecPeriod desc, MalTaxEESysId desc
  end if;
...
/* SubRegistry*/
Update SubRegistry Set DoubleAttr='100' , RegProperty1='RM100' Where SubRegistryId='Denomination11';
