...
begin
  ...
  Declare CurOT double;
  Declare LastOT double;
  ...
  if(IsWageElementInUsed('SubjAdditional','AddWage') = 1) then
    ...
    */
      ...
      select ISNULL(Sum(CurrentOTAmount),0) into CurOT from      
      OTRecord where
      ...
      select ISNULL(Sum(LastOTAmount),0) into LastOT from
      OTRecord where
...
begin
  ...
  if(IsWageElementInUsed('SubjOrdinary','OrdWage') = 1) then
    ...
    */
      ...
      select ISNULL(Sum(LastOTAmount),0) into LastOT from
      OTRecord where
      IsWageElementInUsed('LastOTPay','OrdWage') = 1 and
      IsFormulaIdHasProperty(OTFormulaId,'PreviousRateCPF') = 0 and
      EmployeeSysId = In_EmployeeSysId and
      PayRecYear = In_PayRecYear and
      PayRecPeriod = In_PayRecPeriod and
