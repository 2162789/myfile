if exists(select * from sys.sysprocedure where proc_name = 'ASQLCalHKTaxableItemsB2G') then
...
out Out_TotalPaymentInLieuCode double,
...
begin
  ...
  set Out_TotalPaymentInLieuCode=0;
  ...
  SumLoop1: for SumAllowanceLoop as Cur_SumAllowanceLoop dynamic scroll cursor for
    ...
    if((In_HKTaxRecType <> 'IR56F' and In_HKTaxRecType <> 'IR56G') or
      ...
      if AllowanceFormula = any(select FormulaId from FormulaProperty where KeywordId = 'PaymentInLieuCode') then
        set Out_TotalPaymentInLieuCode=Out_TotalPaymentInLieuCode+AllowanceAmt;
        set NormalAllowance=1
      end if;
  ...
  if Out_TotalPaymentInLieuCode is null then set Out_TotalPaymentInLieuCode=0
  else set Out_TotalPaymentInLieuCode=Round(Out_TotalPaymentInLieuCode,DBDecimal)
  end if;
  ...
  //Insert Payment in Lieu of Notice
  if not exists(select* from HKTaxableItems where EmployeeSysId = In_EmployeeSysId and HKTaxYear = In_HKTaxYear and
      HKTaxRecordType = In_HKTaxRecType and HKTaxItemType = 'PaymentInLieu' and HKTaxItemCode = 'PaymentInLieu') then
    call InsertNewHKTaxableItems(In_EmployeeSysId,In_HKTaxYear,In_HKTaxRecType,'PaymentInLieu','PaymentInLieu','Payment in Lieu of Notice',In_TaxStartDate,In_TaxEndDate,Out_TotalPaymentInLieuCode,0)
  else
    call UpdateHKTaxableItems(In_EmployeeSysId,In_HKTaxYear,In_HKTaxRecType,'PaymentInLieu','PaymentInLieu','Payment in Lieu of Notice',In_TaxStartDate,In_TaxEndDate,Out_TotalPaymentInLieuCode,0,In_OverwriteAdj)
  end if;
...
if not exists(select * from Keyword where KEYWORDID = 'PaymentInLieuCode') then
	...
	Values('PaymentInLieuCode','Payment in Lieu of Notice Code','Payment in Lieu of Notice Code','System',1,0,0,'',0,0,0,'G');
End if;
