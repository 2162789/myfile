...
/* RptCompConfig */
if not exists(select * from RptCompConfig where RptCompSysId = 'Sys_207') then
   insert into RptCompConfig(RptCompSysId,RptConfigId,SysRptId,SysRptCompName)
   values('Sys_207','_Payslip - Laser','Payslip - Laser','YTDFullYearLeave_ComboBox');
end if;

/* RptCompItemConfig */
if not exists(select * from RptCompItemConfig where RptCompSysId = 'Sys_207' and RptCompItemSysId = '1') then
  insert into RptCompItemConfig(RptCompSysId,RptCompItemSysId,ItemValue)
  values('Sys_207','1','YTD Leave');
end if;
