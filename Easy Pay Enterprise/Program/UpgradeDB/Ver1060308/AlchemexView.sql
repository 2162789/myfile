IF EXISTS(SELECT 1 FROM sys.sysprocedure WHERE proc_name ='FGetCareerProgEffectiveDate') THEN
    DROP FUNCTION FGetCareerProgEffectiveDate;
END IF;

create FUNCTION DBA.FGetCareerProgEffectiveDate(
in In_EmployeeSysId integer,
in In_ProgDate date)
returns date
begin
  declare Out_EffectiveDate date;
  select max(CareerEffectiveDate) into Out_EffectiveDate from CareerProgression where EmployeeSysId = in_EmployeeSysId and
    CareerEffectiveDate <= In_ProgDate;
