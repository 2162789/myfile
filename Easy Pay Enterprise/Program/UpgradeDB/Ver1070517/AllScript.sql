...
IF EXISTS (select * from sys.sysprocedure where proc_name = 'FGetWTCalendarDesc') THEN
	DROP FUNCTION FGetWTCalendarDesc;
END IF;

CREATE FUNCTION "DBA"."FGetWTCalendarDesc"(
in In_Id char(20))
returns char(100)
begin
  declare Out_Desc char(100);
  select WTCalendarDesc into Out_Desc
    from WTCalendar where
    WTCalendarId = In_Id;
  return(Out_Desc)
end;
