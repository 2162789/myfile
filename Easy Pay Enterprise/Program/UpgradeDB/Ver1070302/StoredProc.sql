...
where
  ...
  */
  elseif(FGetDBCountry(*) = 'Malaysia') then
    select ContriOrdEECPF+CurrEEManContri+CurrEEVolContri+PrevEEManContri+PrevEEVolContri+ContriAddEECPF into In_TotalContriEECPF
...
in In_ManContriP1Payment smallint,
...
begin
  ...
  elseif not In_MandContriPolicyId = any(select CPFPolicyId from CPFPolicy) then
    ...
    return
  elseif ((not In_MandContriCareerId = any(select CareerId from Career)) and (FGetDBCountry(*) not in ('Indonesia', 'Malaysia'))) then
    set Out_ErrorCode=-4; // MandContriCareerId not exist
    return
  elseif ((not In_MandContriSchemeId = any(select KeywordId from Keyword where KeywordCategory = 'CPFScheme')) and (FGetDBCountry(*) not in ('Indonesia', 'Malaysia'))) then
  ...
  insert into MandatoryContributeProg(EmployeeSysId,MandContriCareerId,MandContriEffDate,MandContriPolicyId,MandContriSchemeId,MandContriRemarks,MandContriCurrent,ManContriActSchemeId,ManContriP1Payment) values(
    In_EmployeeSysId,In_MandContriCareerId,In_MandContriEffDate,In_MandContriPolicyId,In_MandContriSchemeId,In_MandContriRemarks,In_MandContriCurrent,In_ManContriActSchemeId,In_ManContriP1Payment);
  commit work;
  ...
  select MandContriSysId into Out_MandContriSysId from MandatoryContributeProg where
      ...
      ManContriActSchemeId = In_ManContriActSchemeId and 
      ManContriP1Payment = In_ManContriP1Payment;

  if Out_MandContriSysId is null then
     set Out_ErrorCode=0;
     return
  else
    // mark current if this is the first record for that particular scheme
	if FGetDBCountry(*) in ('Indonesia', 'Malaysia') then
...
begin
  ...
  update MandatoryContributeProg set
    ...
    ManContriActSchemeId = In_ManContriActSchemeId,
    ManContriP1Payment = In_ManContriP1Payment
    where MandContriSysId = In_MandContriSysId;
  commit work;
...
begin
  ...
  //
  if FGetDBCountry(*) in ('Malaysia', 'Philippines', 'Indonesia', 'Thailand', 'Brunei') then
...
if exists(select * from sys.sysprocedure where proc_name = 'ASQLAdvReportUpdate') then
  ...
  */
  elseif(In_Country = 'Malaysia') then
    select ContriOrdEECPF, // SOCSO EE Contribution 
      ...
      PrevERVolContri, // Previous ER Voluntary EPF
      ContriAddEECPF, // EIS EE Contribution
      ContriAddERCPF // EIS ER Contribution
      into Out_AdvEEFund1, 
      Out_AdvERFund1,
      Out_AdvEEFund2,
      Out_AdvERFund2,
      ...
      Out_AdvERFund5,
      Out_AdvEEFund6,
      Out_AdvERFund6 from PolicyRecord where EmployeeSysId = In_EmployeeSysId and
      PayRecYear = In_PayRecYear and
      PayRecPeriod = In_PayRecPeriod and
      PayRecSubPeriod = In_PayRecSubPeriod and
      PayRecID = In_PayRecordID
