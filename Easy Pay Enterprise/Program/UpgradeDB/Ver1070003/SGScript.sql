/* HSBC (Net G3) */
if not exists (select 1 from BankSubmitFormat where BankSubmitSubmitForId = 'Salary' and FormatName = 'HSBC (Net G3)') then
  insert into BankSubmitFormat (BankSubmitSubmitForId, FormatName, DllName, FormatterInvoke, IsCustomised)
  values ('Salary', 'HSBC (Net G3)', 'RSingBankFormatHSBCNetG3.dll', 'InvokeSalaryFormatter', 0);
end if;

/* S2B Lite G3 */
if not exists (select 1 from BankSubmitFormat where BankSubmitSubmitForId = 'Salary' and FormatName = 'Standard Chartered (S2B Lite G3)') then
  insert into BankSubmitFormat (BankSubmitSubmitForId, FormatName, DllName, FormatterInvoke, IsCustomised)
  values ('Salary', 'Standard Chartered (S2B Lite G3)', 'RSingBankFormatStandardCharteredS2BG3.dll', 'InvokeSalaryFormatter', 0);
end if;

/* YE Changes */
if not exists(select 1 from sys.syscolumns where tname='IR21A1S4S5' and cname='Address1PaidByEmployee') then
    alter table DBA.IR21A1S4S5 Add Address1PaidByEmployee double;
end if;

if not exists(select 1 from sys.syscolumns where tname='IR21A1S4S5' and cname='Address2PaidByEmployee') then
    alter table DBA.IR21A1S4S5 Add Address2PaidByEmployee double;
end if;
...
In In_Address1PaidByEmployee double, 
In In_Address2PaidByEmployee double, 
...
BEGIN
  if not exists(select* from IR21A1S4S5 where
    ...
    insert into IR21A1S4S5(PersonalSysId,
      ...
      Address1PaidByEmployee, 
      Address2PaidByEmployee,
      PUBTelPagerValue, 
      Driver, 
      ServantGardener, 
      HotelAccommodation, 
      ...
      In_Address1PaidByEmployee,
      In_Address2PaidByEmployee,
      In_PUBTelPagerValue, 
      In_Driver, 
      In_ServantGardener, 
...
BEGIN
      if exists(select * from IR21A1S4S5 Where PersonalSysId = In_PersonalSysId And YEYear = In_YEYear) Then
	    Update IR21A1S4S5 Set
         ...
         Address1PaidByEmployee = In_Address1PaidByEmployee,
         Address2PaidByEmployee = In_Address2PaidByEmployee,
...
if not exists (select 1 from YEKeyWord where YEKeywordCategory='IncomeType' and YEKeyWordId = 'IncomeTypeOPFTC') then
  ...
  values ('IncomeTypeOPFTC', 'Overseas Pension Fund with Tax Concession(OPFTC)', 'Overseas Pension Fund with Tax Concession(OPFTC)', 'IncomeType','',5,'',0,0,0,0,0,0,'1899-12-30 00:00:00');
end if;

if not exists (select 1 from YEKeyWord where YEKeywordCategory='IncomeType' and YEKeyWordId = 'IncomeTypeOE') then
  insert into YEKeyWord (YEKeyWordId, YEKeyWordDefaultName,YEKeyWorduserDefinedName,YEKeyWordCategory,YEKeywordDesc,YEProperty1,YEProperty2,YEProperty3,YEProperty4,YEProperty5,YEProperty6,YEProperty7,YEProperty8,YEProperty9)
  values ('IncomeTypeOE', 'Overseas Employment(OE)', 'Overseas Employment(OE)', 'IncomeType','',6,'',0,0,0,0,0,0,'1899-12-30 00:00:00');
end if;

if not exists (select 1 from YEKeyWord where YEKeywordCategory='IncomeType' and YEKeyWordId = 'IncomeTypeOEOPFTC') then
  insert into YEKeyWord (YEKeyWordId, YEKeyWordDefaultName,YEKeyWorduserDefinedName,YEKeyWordCategory,YEKeywordDesc,YEProperty1,YEProperty2,YEProperty3,YEProperty4,YEProperty5,YEProperty6,YEProperty7,YEProperty8,YEProperty9)
  values ('IncomeTypeOEOPFTC', 'Overseas Employment AND Overseas Pension Fund with Tax Concession(OEOPFTC)', 'Overseas Employment AND Overseas Pension Fund with Tax Concession(OEOPFTC)', 'IncomeType','',7,'',0,0,0,0,0,0,'1899-12-30 00:00:00');
end if;
